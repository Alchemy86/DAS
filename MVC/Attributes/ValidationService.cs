    ...
    public class ValidationService : ActionFilterAttribute
    {
        ...
        public void RedirectToLoginIfNoSession()
        {
            ...
            else
            {
                ...
                if ("Verified" != AccountVerified && controller.Request.Method != "POST")
                {
                    context.Result = RedirectToSettings();
                }
            }
        }

        private Guid CascadeSessionToken
            => Session.GetString("SessionToken") == null ? Guid.Empty : new Guid(Session.GetString("SessionToken"));

        protected string Username => Session.GetString("DASUserName") ?? string.Empty;

        protected string AccountVerified => Session.GetString("AccountVerified") ?? string.Empty;
        ...
        private IActionResult LoginOverride(string message)
        {
            ...
            controller.ViewBag.Warning = string.IsNullOrEmpty(message)
                ? "Your session has expired or you do not have access to the page or function you just tried to access."
                : message;
            return controller.View("~/Views/Login/Index.cshtml");
        }

        private IActionResult RedirectToSettings()
        {
            controller.ViewBag.Warning =
                "Your Godaddy Account is required to place bids";

            return controller.View("~/Views/Settings/Index.cshtml");
        }
    }
}
