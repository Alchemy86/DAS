...
namespace DAS.GoDaddyv2
{
    public class GoDaddyAuctionSniper : HttpBase
    {
        ...
        public bool Login(int attempNo = 0, string username = null, string password = null)
        {
            ...
            const string loginurl = "https://sso.godaddy.com/?realm=idp&app=auctions&path=/";
            var hdoc = HtmlDocument(Post(loginurl,
                $"name={username ?? sessionDetails.GoDaddyAccount.Username}&password={password ?? sessionDetails.GoDaddyAccount.Password}"));
            ...
            if (QuerySelector(hdoc.DocumentNode, "img[class='LBD_CaptchaImage']") != null)
            {
                ...
                try
                {
                    imagedata.Save(Path.Combine(Path.GetTempPath(), username ?? sessionDetails.GoDaddyAccount.Username + ".jpg"), ImageFormat.Jpeg);
            ...
            if (QuerySelectorAll(doc.DocumentNode, "tr.srRow2, tr.srRow1") != null)
            {
                foreach (var node in QuerySelectorAll(doc.DocumentNode, "tr.srRow2, tr.srRow1"))
                {
                    if (QuerySelector(node, "span.OneLinkNoTx") != null && QuerySelector(node, "td:nth-child(5)") != null)
                    {
                        ...
                        catch (System.Exception) { auction.BuyItNow = 0; }
                        ...
                        if (auction.MinBid > 0)
                        {
                            auctions.Add(new Auction(auction.AuctionID, auction.EndDate, auction.EstimateEndDate, auction.DomainName, auction.AuctionRef, auction.BidCount, auction.MinBid, auction.MyBid, auction.Processed, auction.Traffic));
                        }
                    }
                }
            }
        ...
        public void PlaceBid(Auction auction)
        {
            ...
            if (Login())
            {
                ...
                postData = "__VIEWSTATE=" +
                           ("&hidAdvSearch=ddlAdvKeyword%3A1%7CtxtKeyword%3Aportal" +
